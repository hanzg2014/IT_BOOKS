...
public class Ch10_1_1HoughLine {
	...
	private void initialize() {
		final Mat source = Highgui.imread("C://opencv3.1//samples//building.jpg");
		
		BufferedImage image=matToBufferedImage(source);
		
		frmjavaSwing = new JFrame();
		frmjavaSwing.setTitle("opencv ÀN¤Ò½uÅÜ´«1");
		...
		slider_CT1.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(Canny(source,slider_CT1.getValue(),slider_CT2.getValue()));
		...
		slider_srn.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(HoughLine(source,slider_ksize.getValue(),slider_srn.getValue(),slider_stn.getValue(),slider_CT1.getValue(),slider_CT2.getValue()));
				lblNewLabel.setIcon(new ImageIcon(newImage));
			}
		});
	...
	public Mat HoughLine(Mat source,int threshold,double srn,double stn,double Canny_threshold1,double Canny_threshold2){
		 Mat mRgba = new Mat();
		 ...
		 for (int x = 0; x < lines.total(); x++) 
		    {
			      ...
			      start=new Point(x0+1000*(-b), y0+1000*a);
		          	...
		          	System.out.println("line"+x+":x=("+(x0+1000*(-b))+","+(y0+1000*a)+"),y=("+(x0-1000*(-b))+","+(y0-1000*a)+")");
		          Core.line(destination, start, end, new Scalar(255,0,0), 3);
