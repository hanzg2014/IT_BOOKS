...
public class Ch11_1_1ShiTomasi {
	...
	private void initialize() {
		final Mat source = Imgcodecs.imread("C://opencv3.1//samples//lena.jpg",Imgcodecs.CV_LOAD_IMAGE_COLOR);
		
		BufferedImage image=matToBufferedImage(source);
		
		frmjavaSwing = new JFrame();
		frmjavaSwing.setTitle("Shi-Tomasi¨¤ÂIÀË´ú¤èªk");
		...
		slider_ql.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(ShiTomasi(slider_mc.getValue(),(float)slider_ql.getValue()/100,slider_md.getValue()));
	...
	public Mat ShiTomasi(int maxCorners, double qualityLevel, double minDistance){
		Mat source = Imgcodecs.imread("C://opencv3.1//samples//lena.jpg",Imgcodecs.CV_LOAD_IMAGE_COLOR);
		Mat destination=source.clone();
		 Imgproc.cvtColor(destination, destination, Imgproc.COLOR_BGR2GRAY);
         MatOfPoint corners = new MatOfPoint();
       
         Imgproc.goodFeaturesToTrack(destination, corners, maxCorners, qualityLevel, minDistance);
